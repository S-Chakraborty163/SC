...
class AnimationController {
  ...
  // Exposed read-only accessors so Star.render() doesn't need to reach into private fields
  public get cameraZOffset() {
    return this.cameraZ;
  }
  public get viewZoomFactor() {
...
class Star {
  ...
  render(p: number, controller: AnimationController) {
    ...
    if (q > 0) {
      ...
      const vx = (this.z - controller.cameraZOffset) * screenX / controller.viewZoomFactor;
      const vy = (this.z - controller.cameraZOffset) * screenY / controller.viewZoomFactor;
...
type SpiralAnimationProps = Omit<React.ComponentProps<"div">, "ref">;

export function SpiralAnimation({ className, ...props }: SpiralAnimationProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationRef = useRef<AnimationController | null>(null);
  const [size, setSize] = useState(0);

  // Measure the container (not the window) so this stays scoped to its section
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      setSize(Math.max(clientWidth, clientHeight, 1));
    };
  ...
  // Build (and rebuild on resize) the canvas + animation controller
  useEffect(() => {
  ...
  return (
    <div
      ref={containerRef}
      className={cn("pointer-events-none absolute inset-0 overflow-hidden flex items-center justify-center", className)}
      {...props}
    >
      <canvas
        ref={canvasRef}
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 scale-[0.45]"
      />
    </div>
  );
}
