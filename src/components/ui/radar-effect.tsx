...
export const Radar = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-transparent via-primary/50 to-transparent" />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(124, 58, 237, ${0.4 - (idx + 1) * 0.05})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
...
export const IconContainer = ({
  ...
  return (
    ...
    >
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl group transition-all hover:border-primary/50 hover:bg-primary/10">
        {icon || (
      ...
      <div className="rounded-md px-2 py-1">
        <div className="text-center text-[10px] font-bold text-white/50 uppercase tracking-widest">
          {text || "Project"}
        </div>
      </div>
    </motion.div>
