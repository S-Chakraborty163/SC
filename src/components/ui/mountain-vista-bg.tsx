...
const MountainVistaParallax = ({ title = '', subtitle = '' }: MountainVistaParallaxProps) => {
  ...
  return (
    <div
      className="hero-container-parallax"
    ...
    >
      ...
      {/* Optional Hero text - only if title or subtitle provided */}
      {(title || subtitle) && (
        <div className="hero-content-parallax">
          <h1 className="hero-title-parallax">{title}</h1>
          <p className="hero-subtitle-parallax">{subtitle}</p>
        </div>
      )}
    </div>
  );
