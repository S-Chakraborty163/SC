...
import { ExternalLink, Github, Car, Bot, Tv, TrendingUp, ScanFace, Landmark } from "lucide-react";
import CyberneticGridShader from "@/components/ui/cybernetic-grid-shader";
import { Radar, IconContainer } from "@/components/ui/radar-effect";

const projects = [
  {
    title: "Sansad-v2: Parliamentary Policy Graph-RAG",
    description: "Building an end-to-end Graph-RAG system for Indian parliamentary policy analysis, combining DuckDB, ChromaDB, and Neo4j with a custom LoRA fine-tuned Mistral-7B model. Ongoing research internship project at IIT Indore.",
    highlights: [
      "92.89% Faithfulness, 91.18% Citation Accuracy (double-blind eval)",
      "Neo4j knowledge graph — 76K+ entities, ~40M relationships",
      "Two-stage LoRA fine-tuning, quantized to 4-bit GGUF via Ollama"
    ],
    icon: <Landmark className="h-8 w-8 text-primary" />,
    tags: ["Neo4j", "ChromaDB", "LoRA", "Streamlit"],
    github: "https://github.com/S-Chakraborty163/Application-of-AI-in-Parliamentary-sessions",
    inProgress: true,
  },
  {
      ...
      <div className="container relative z-10 px-6">
        ...
        <div className="relative flex h-[400px] w-full items-center justify-center mb-32 overflow-hidden">
           <div className="absolute top-0 flex w-full max-w-4xl items-center justify-between px-10">
             <IconContainer text="CV" delay={0.1} icon={projects[1].icon} />
             <IconContainer text="RAG" delay={0.2} icon={projects[2].icon} />
             <IconContainer text="NLP" delay={0.3} icon={projects[3].icon} />
           </div>
           <div className="flex w-full max-w-2xl items-center justify-around">
             <IconContainer text="Finance" delay={0.4} icon={projects[4].icon} />
             <IconContainer text="Deep Learning" delay={0.5} icon={projects[5].icon} />
           </div>
          ...
          {projects.map((project, index) => (
            <Card key={project.title} className="bg-white/5 backdrop-blur-xl border-white/5 flex flex-col overflow-hidden hover:border-primary/20 transition-all duration-500 group">
              <CardHeader className="p-8">
                ...
                <CardTitle className="text-2xl font-bold text-white mb-4 group-hover:text-primary transition-colors flex items-center gap-3 flex-wrap">
                  {project.title}
                  {project.inProgress && (
                    <Badge variant="outline" className="text-[9px] uppercase tracking-wider border-amber-400/30 bg-amber-400/10 text-amber-400 font-bold">
                      🚧 In Progress
                    </Badge>
                  )}
                </CardTitle>
